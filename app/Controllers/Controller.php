...
abstract class Controller {
    ...
    protected $csrf;
    protected $currentLang = 'tw'; // 【新增】當前語系
    ...
    public function __construct(ContainerInterface $container) {
        ...
        $this->currentLang = $this->view->getAttribute('lang') ?? 'tw';
    ...
    protected function getDefaultLanguage() {
        ...
        // 使用 config 中定義的預設語系作為回退值
        global $defaultLang;
        return $defaultLang ?? 'tw';
    }
    ...
    protected function render($response, $template) {
        global $imgPathFormat, $templatePath;
        ...
        // 使用 config 中的圖片路徑格式
        $frontImgPath = $baseurl . str_replace('{lang}', $this->currentLang, $imgPathFormat);

        // 使用 config 中的模板路徑
        $tpl_setting = $this->systemTemplateSet;
        $tpl_setting['Template'] = new \App\Helpers\TemplateHelper($templatePath);

        $htmlLang = $this->currentLang;
