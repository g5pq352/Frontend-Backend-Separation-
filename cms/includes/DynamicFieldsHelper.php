...
class DynamicFieldsHelper
{
    ...
    private function isGroupDirty($new, $old, $fieldConfig)
    {
        foreach ($fieldConfig as $f) {
            ...
            if ($f['type'] === 'image') {
                if ($this->isImageFieldDirty($newVal, $oldRow, $f)) {
                    return true;
                }
            } else {
     ...
     * 檢查圖片欄位是否有變更
     */
    private function isImageFieldDirty($newVal, $oldRow, $fieldConfig)
    {
        $isMultiple = $fieldConfig['multiple'] ?? false;

        if ($isMultiple) {
            return $this->isMultipleImageDirty($newVal, $oldRow);
        } else {
            return $this->isSingleImageDirty($newVal, $oldRow);
        }
    }
     ...
     * 檢查單圖欄位是否有變更
     */
    private function isSingleImageDirty($newVal, $oldRow)
    {
        // 前端「移除圖片」模式 1: 直接送 '__DELETE__'
        if ($newVal === '__DELETE__') {
            $oldFileId = is_array($oldRow) ? ($oldRow['df_file_id'] ?? null) : null;
            return !empty($oldFileId);
        }

        // 前端「移除圖片」模式 2: 送 array，且 [file_id] 為 '__DELETE__'
        if (is_array($newVal) && ($newVal['file_id'] ?? null) === '__DELETE__') {
            $oldFileId = is_array($oldRow) ? ($oldRow['df_file_id'] ?? null) : null;
            return !empty($oldFileId);
        }

        // 沒有新值 (可能完全沒選、或只改標題但 file_id 沒變)
        if (is_array($newVal)) {
            ...
            // 如果 file_id 沒變，但標題有送且跟舊的不一樣，也是 dirty
     ...
     * 檢查多圖欄位是否有變更
     */
    private function isMultipleImageDirty($newVal, $oldRow)
    {
        $newImages = is_array($newVal) ? $newVal : [];
        $oldImages = is_array($oldRow) ? $oldRow : ($oldRow ? [$oldRow] : []);
        ...
        foreach ($newImages as $idx => $img) {
            $newFileId = is_array($img) ? ($img['file_id'] ?? null) : $img;
            $oldFileId = isset($oldImages[$idx]) ?
                (is_array($oldImages[$idx]) ? ($oldImages[$idx]['file_id'] ?? null) : $oldImages[$idx]) :
                null;
    ...
    private function updateGroupByUidDiff($d_id, $fieldGroup, $uid, $gi, $new, $old, $fieldConfig)
    {
        foreach ($fieldConfig as $f) {
            ...
            if ($f['type'] === 'image') {
                $this->updateImageField($d_id, $fieldGroup, $uid, $gi, $name, $newVal, $oldRow, $f);
            } else {
     ...
     * 更新圖片欄位
     */
    private function updateImageField($d_id, $fieldGroup, $uid, $gi, $name, $newVal, $oldRow, $fieldConfig)
    {
        $isMultiple = $fieldConfig['multiple'] ?? false;

        if ($isMultiple) {
            // 多圖模式
            $this->updateMultipleImages($d_id, $fieldGroup, $uid, $gi, $name, $newVal, $oldRow);
        } else {
            // 單圖模式
            $this->updateSingleImage($d_id, $fieldGroup, $uid, $gi, $name, $newVal, $oldRow);
        }
    }
     ...
     * 更新單圖欄位
     */
    private function updateSingleImage($d_id, $fieldGroup, $uid, $gi, $name, $newVal, $oldRow)
    {
        $newFileId = is_array($newVal) ? ($newVal['file_id'] ?? null) : $newVal;
        $oldFileId = is_array($oldRow) ? ($oldRow['df_file_id'] ?? null) : null;
        $newTitle  = is_array($newVal) ? ($newVal['title'] ?? null) : null;

        // 刪圖：前端會送 null 或 '__DELETE__'
     ...
     * 處理多圖欄位的更新 (簡化版)
     * ========================= */
    private function updateMultipleImages($d_id, $fieldGroup, $uid, $gi, $fieldName, $newImages, $oldImages)
    {
        // 標準化新圖片資料
    ...
    private function isImageField($name, $fieldConfig)
    {
        foreach ($fieldConfig as $f) {
