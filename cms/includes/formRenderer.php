...
function renderTextField($config, $value = '')
{
    ...
    $html .= "<input name=\"{$field}\" type=\"text\" class=\"form-control form-control-modern\" id=\"{$field}\" value=\"{$value}\" size=\"{$size}\" {$readonly} {$requiredAttr} {$checkDuplicate} />";
    if ($note) {
...
function renderPasswordField($config, $value = '')
{
    ...
    $html .= "<label class=\"col-lg-5 col-xl-2 control-label text-lg-end mb-0\">{$label}{$requiredMark}</label>";
    $html .= "<div class=\"col-lg-7 col-xl-7\">";
    $html .= "<input name=\"{$field}\" type=\"password\" class=\"form-control form-control-modern\" id=\"{$field}\" value=\"\" size=\"{$size}\" placeholder=\"留空表示不修改\" />";
    if ($note) {
        $html .= "<label class=\"error mt-2\">{$note}</label>";
    }
    $html .= "</div>";
...
function renderSelect($config, $value = '')
{
    ...
    if ($value === '' || $value === null) {
        if (isset($config['default'])) {
            ...
            $value = htmlspecialchars($_GET[$field], ENT_QUOTES, 'UTF-8');
        }
    }
    ...
    $html = "<div class='form-group row align-items-center pb-3'>";
        ...
        $html .= "<div class=\"col-lg-7 col-xl-7\">";
            ...
            elseif (isset($config['options'])) {
                ...
                $html .= "<select name=\"{$field}\" id=\"{$field}\" class=\"{$class}\" {$requiredAttr}>";

                foreach ($config['options'] as $option) {
                    $selected = ((string)$option['value'] === (string)$value) ? 'selected' : '';
                    $iconClass = $option['icon'] ?? $option['value'];
                    $html .= "<option value=\"{$option['value']}\" data-icon=\"{$iconClass}\" {$selected}>{$option['label']}</option>";
                }
                $html .= "</select>";
            }
        if ($note) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data && data.length > 0) {
                            ...
                            nextSelect += '<option value=\"\">-- 請選擇 --</option>';
