...
class DynamicFieldsEditor {
    ...
    renderMultipleImageField(fieldConfig, value, groupIndex, group, uid) {
        ...
        html += `
                ...
                ${finalNote ? `<div style="margin-top:5px;"><span class="table_data" style="color:#d9534f;">${finalNote}</span></div>` : ''}
            </div>
        `;
    ...
    syncDataFromDom() {
        ...
        items.forEach((item, loopIndex) => {
            // ⭐ 關鍵修正：使用 dataset.groupIndex 來找到對應的原始資料，而不是迴圈索引
            // 因為在拖曳排序時，DOM 的順序已經變了，但 this.groups 還沒變
            ...
            this.config.fields.forEach(field => {
                const fieldName = `${this.config.field}[${loopIndex}][${field.name}]`; // input name still follows DOM order? No wait. 
                const originalFieldName = `${this.config.field}[${originalIndex}][${field.name}]`;

                if (field.type === 'image') {
                    // 讀取隱藏的 file_id 欄位
                    const input = item.querySelector(`input[name*="[${field.name}][file_id]"]`);
                    if (input) {
                        ...
                        if (typeof groupData[field.name] === 'object' && groupData[field.name] !== null) {
                            ...
                            // 如果是移除圖片，順便清空 file_info 避免重新渲染時又畫出圖
                    ...
                    if (titleInput && typeof groupData[field.name] === 'object' && groupData[field.name] !== null) {
                        if (!groupData[field.name].file_info) {
                            groupData[field.name].file_info = {};
                        }
                        groupData[field.name].file_info.file_title = titleInput.value;
                    }
                } else {
                    // 文字欄位
                    // Match by attribute ending in [field.name]
